...
// Contains only questions without answers or solution steps
...
export const prebuiltQuestions: Record<string, Record<string, PrebuiltQuestion[]>> = {
  "Algebra": {
    "Linear Equations": [
      {
        id: "alg-lin-1",
...
export const getQuestionsByFilters = (
  ...
  if (prebuiltQuestions[topic]) {
    subtopics.forEach(subtopic => {
      if (prebuiltQuestions[topic][subtopic]) {
        // Filter by difficulty
        const questionsWithDifficulty = prebuiltQuestions[topic][subtopic].filter(
  ...
  if (filteredQuestions.length < count) {
    Object.values(prebuiltQuestions[topic] || {}).forEach(questions => {
  ...
  if (filteredQuestions.length < count) {
    Object.values(prebuiltQuestions).forEach(topicQuestions => {
...
// Answers and solution steps for the questions - these will be used separately
export const questionAnswers: Record<string, { answer: string, steps: string[] }> = {
  "alg-lin-1": {
    answer: "x = 5",
    steps: [
